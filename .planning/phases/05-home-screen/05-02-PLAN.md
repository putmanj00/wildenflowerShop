...
export default function HomeScreen() {
  ...
  const featuredHook = useProducts(
    useCallback(() => ({ collection: 'featured', limit: 6 }), [])
  );
...
**IMPORTANT — useProducts hook signature check:**

Read `hooks/useProducts.ts` before writing to confirm the exact signature of `useProducts`. The RESEARCH.md documents it as `useProducts(options?)` with `{ collection?, limit? }` — confirm whether it accepts a plain options object or a callback. If it takes a plain object (not useCallback), drop the `useCallback` wrapping and pass `{ collection: 'featured', limit: 6 }` directly. Honor whatever the actual implementation uses — do NOT change the hook.
...
- Two `useProducts` hooks provide featured-first-with-fallback pattern
